...
from swx_api.core.utils.model import load_all_models  # Ensure it prevents duplicate loading
...
# ✅ Prevent multiple model imports
if not SQLModel.metadata.tables:
    load_all_models()

# ✅ Debugging: Print detected models
print(f"🔍 Alembic detected models: {list(SQLModel.metadata.tables.keys())}")

# Use SQLModel metadata for Alembic
target_metadata = SQLModel.metadata
...
# Run Alembic migrations based on execution mode
if context.is_offline_mode():
    run_migrations_offline()
else:
    run_migrations_online()
