...
# from both `swx_app.models` and `swx_core.models` to ensure
...
import pkgutil
import importlib
from swx_core.models.base import Base  # Adjust as needed


def load_all_models() -> Base:
    """
       Dynamically loads (and reloads) all SQLAlchemy models from both `swx_app.models`
       and `swx_core.models` so that they register with `Base.metadata`.

       Behavior:
           - Scans `swx_app.models` and `swx_core.models` for SQLAlchemy models.
    ...
    modules_to_scan = ["swx_app.models", "swx_core.models"]
