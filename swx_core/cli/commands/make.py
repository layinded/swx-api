...
from swx_core.utils.helper import (
    ...
    create_file, get_extra_schemas,
)
from swx_core.utils.model import load_all_models
from swx_core.cli.commands.security_validation import (
    validate_field_name,
...
@click.argument("name")
@click.option(
    "--module", default="swx_app.models", help="Module where the model is defined"
)
def controller(name, module):
    """Generate a controller based on an existing model."""
    # Security: Validate resource name and module path
    is_valid, error = validate_resource_name(name)
    if not is_valid:
        click.secho(f"❌ Security Error: {error}", fg="red")
        return
    
    is_valid, error = validate_module_path(module)
    if not is_valid:
        click.secho(f"❌ Security Error: {error}", fg="red")
        return
...
def model(name, columns, module):
    ...
    # Security: Final validation of all columns
...
def make_from_model(ctx: Context, model_name, module):
    ...
    """
    # Security: Validate resource name and module path
    is_valid, error = validate_resource_name(model_name)
    if not is_valid:
        click.secho(f"❌ Security Error: {error}", fg="red")
        return
...
def make_model_from_existing(model_name, module):
    ...
    content = (
        ...
        f"from swx_core.models.base import Base\n\n"
...
def resource(resource_name, module, columns, migration, existing):
    ...
    load_all_models()
    from swx_core.models.base import Base  # adjust based on your project structure
