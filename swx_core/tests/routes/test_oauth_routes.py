...
from swx_core.main import app  # Import FastAPI app instance
...
@patch("swx_core.config.social_settings.ENABLE_GOOGLE_LOGIN", True)
@patch(
    "swx_core.config.social_settings.GOOGLE_REDIRECT_URI",
    "https://example.com/callback",
)
@patch("swx_core.config.social_settings.GOOGLE_CLIENT_ID", "mock_client_id")
@patch("swx_core.config.social_settings.GOOGLE_CLIENT_SECRET", "mock_client_secret")
@patch(
    "swx_core.routers.oauth.oauth.google.authorize_redirect", new_callable=AsyncMock
)
...
@patch(
    "swx_core.routers.oauth.oauth.google.authorize_access_token",
    new_callable=AsyncMock,
)
@patch("swx_core.repositories.user_repository.get_user_by_email", return_value=None)
@patch(
    "swx_core.repositories.user_repository.create_social_user",
    return_value=User(email="test@example.com"),
)
@patch("swx_core.controllers.auth_controller.login_social_user_controller")
...
@patch("swx_core.config.social_settings.ENABLE_FACEBOOK_LOGIN", True)
@patch(
    "swx_core.config.social_settings.FACEBOOK_REDIRECT_URI",
    "https://example.com/callback",
)
@patch(
    "swx_core.routers.oauth.oauth.facebook.authorize_redirect",
    new_callable=AsyncMock,
)
...
@patch(
    "swx_core.routers.oauth.oauth.facebook.authorize_access_token",
    new_callable=AsyncMock,
)
@patch("swx_core.routers.oauth.fetch_facebook_user_info", new_callable=AsyncMock)
@patch("swx_core.repositories.user_repository.get_user_by_email", return_value=None)
@patch(
    "swx_core.repositories.user_repository.create_social_user",
    return_value=User(email="test@example.com"),
