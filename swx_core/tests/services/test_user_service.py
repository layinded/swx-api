...
from swx_core.models.user import User, UserUpdate, UserUpdatePassword
...
def test_update_user_profile_service(test_db, mock_user, mock_request):
    ...
    with patch(
        "swx_core.repositories.user_repository.update_user", return_value=mock_user
    ):
...
@patch("swx_core.utils.language_helper.translate", return_value="User not found")
def test_get_user_by_id_service_not_found(mock_translate, mock_request, test_db):
    """Test getting a non-existent user raises 404."""
    with patch(
        "swx_core.repositories.user_repository.get_user_by_id", return_value=None
    ):
...
@patch("swx_core.repositories.user_repository.get_user_by_id")
...
def test_update_password_service_failure(
    ...
    with patch(
        "swx_core.repositories.user_repository.update_user_password",
        return_value=False,
    ):
        with pytest.raises(HTTPException) as exc_info:
...
@patch(
    "swx_core.utils.language_helper.translate", return_value="User deletion failed"
)
def test_delete_user_service_failure(mock_translate, test_db, mock_user, mock_request):
    """Test deleting a user that does not exist."""
    with patch(
        "swx_core.repositories.user_repository.delete_user", return_value=False
    ):
        with pytest.raises(HTTPException) as exc_info:
...
def test_delete_user_service_success(mock_translate, test_db, mock_user, mock_request):
    ...
    with patch(
        "swx_core.repositories.user_repository.delete_user", return_value=True
    ):
